...
1. ✅ **실제 실행 경로 전환**: LocalContainerService에서 명령 실행을 `CommandRuntime`으로 추상화하고, CloudContainerService가 `DockerCommandRuntime`을 주입해 `docker exec -i --workdir … --env …` 방식으로 모든 ExecutorAction을 컨테이너 안에서 실행. MsgStore/승인 파이프라인은 기존과 동일하게 유지됨.
2. ✅ **프로세스 생명주기 관리**: 각 attempt에 대한 provisioned container/secret 디렉터리를 `DashMap`으로 추적하고, 기존 `AsyncGroupChild` 관리(try_stop/kill)가 docker exec 프로세스를 종료하도록 재사용. teardown 시 컨테이너 및 비밀 디렉터리를 제거.
3. ✅ **비밀 주입 흐름 구체화**: SecretStore → `/tmp/anyon/cloud-secrets/<attempt>` → 컨테이너 `/tmp/anyon-secrets` bind mount 패턴으로 Claude config/Git credential 파일을 작성하고, `CLAUDE_CONFIG_PATH`/`GIT_CONFIG_GLOBAL`/`GH_TOKEN` env를 docker exec `--env`로 전달. 실행 종료 시 비밀 디렉터리를 삭제.
